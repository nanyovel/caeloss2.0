...
export const DetalleFurgon = ({
  ...
  const buscarDoc = (e) => {
    ...
    if (validacion.docExiste == true && validacion.hasNumero == true) {
      setRefresh(!refresh);
      navigate("/importaciones/maestros/contenedores/" + buscarDocInput);
      setBuscarDocInput("");
    }
  };
  ...
  const handleChange = async (e) => {
    ...
    if (yaExistia == false) {
      ...
      let seguimiento = furgonUpdate.seguimientos
        ? furgonUpdate.seguimientos
        : [];
      seguimiento.push({
        activo: checK,
    ...
    <>
      ...
      <CajaEncabezado>
        <CajaDetalles>
          ...
          <CajitaDetalle>
            <TituloDetalle
              className={`${furgonMaster.diasRestantes < 2 ? "negativo" : ""}
