...
export const DetalleOrden = ({ dbOrdenes, userMaster, usuario }) => {
  ...
  useEffect(() => {
    ...
    if (
      matSombra.every((articulo) => {
        return articulo.cerrado == 1;
      })
    ) {
      setEstadoDoc(1);
    }
    ...
    // Si alguno de sus items tiene qty pendiente
      ...
      matSombra.some((articulo) => {
        return articulo.cerrado == 0;
      })
    ) {
      setEstadoDoc(0);
    }
    ...
    for (const orden of dbOrdenes) {
      const existeNumero = dbOrdenes.some(
        (orden) => orden.numeroDoc == `E${[num]}${ocMaster.numeroDoc}`
      );
    ...
    if (e.target.dataset.nombre == "eliminarDoc") {
      if (
        ...
        setMensajeAdvertencia(
          "¿Seguro que desea eliminar este esta Orden de Compra?"
        );
    ...
    inputs: [
      {
        ...
        comentarios: "",
        qtyDisponible: "",
        despachos: "empty por edicion detalle Orden",
      },
      {
        n: "",
        codigo: "",
  ...
  const guardarCambios = async () => {
    ...
    const matMasDespacho = materialesParsed.map((item) => {
      if (Array.isArray(item.despachos) && item.despachos.length > 0) {
        return item;
      } else {
        return {
          ...item,
  ...
  const handleChange = async (e) => {
    ...
    if (yaExistia == false) {
      ...
      let seguimiento = ordenUpdate.seguimientos
        ? ordenUpdate.seguimientos
        : [];
      seguimiento.push({
        activo: checK,
      ...
      <CajaEncabezado>
        <CajaDetalles>
          ...
          <CajitaDetalle>
            ...
            {isEditando == false ? (
              <DetalleTexto title={ocMaster.proveedor}>
                {ocMaster.proveedor}
              </DetalleTexto>
            ) : (
              <InputEditable
          ...
          <CajitaDetalle>
            ...
            {isEditando == false ? (
              <DetalleTexto title={ocMaster.comentarios}>
                    ...
                    ocMaster.materiales.map((item, index) => {
                      ...
                      if (index < ocMaster?.materiales?.length) {
                        if (index < ocMaster?.materiales.length) {
                          ...
                          if (
                            ocMaster.materiales[index].despachos?.length > 0
                          ) {
                            despachos.map((desp) => {
                              cantidadDespachada += desp.qty;
                            });
