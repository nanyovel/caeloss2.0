...
export const DetalleBL = ({
  ...
  useEffect(() => {
    ...
    if (blMaster.furgones.length > 0) {
      ...
      if (blMaster.furgones.some((furgon) => furgon.status < 5) == true) {
        ...
        blMaster.furgones.every((furgon) => furgon.status >= 5) == true
      ) {
        setEstadoDoc(1);
      }
      ...
      for (const bl of dbBillOfLading) {
        const existeNumero = dbBillOfLading.some(
          (bl) => bl.numeroDoc == `E${[num]}${blMaster.numeroDoc}`
        );
    ...
    if (e.target.dataset.nombre == "eliminarFila") {
      ...
      if (
        ...
        setMensajeAdvertencia(
          "¿Seguro que desea eliminar este Bill Of Lading?"
        );
        setDispatchAdvertencia(true);
        setEventFunction(e);
  ...
  const buscarDoc = (e) => {
    ...
    const blBuscar = dbBillOfLading.find(
      (bl) => bl.numeroDoc.toUpperCase() == buscarDocInput.toUpperCase()
    );
    if (blBuscar) {
      docExiste = true;
    }
    ...
    // Si todo esta correcto
      ...
      navigate("/importaciones/maestros/billoflading/" + buscarDocInput);
      setBuscarDocInput("");
      setRefresh(!refresh);
    }
  };
  ...
  const eliminarDoc = async () => {
    ...
    const ordenesUsadas = [
      ...new Set(
        blMaster.furgones.flatMap((furgon) =>
    ...
    try {
      ...
      setTimeout(() => {
        navigate("/importaciones/maestros/billoflading/");
      }, 500);
    } catch (error) {
  ...
  const agregarMateriales = (e) => {
    ...
    if (validacion.sinCambios == true) {
      ...
      setCopiarAFurgonMaster(
        blEditable.furgones[index].materiales.map((articulo) => {
          let valoresParsed = {};
          dbOrdenes.map((orden) => {
  ...
  const [furgonEditable, setFurgonEditable] = useState(
  ...
  const guardarCambios = async () => {
    ...
    dbBillOfLading.forEach((bl) => {
      if (
        bl.numeroDoc == blEditable.numeroDoc &&
        blEditable.numeroDoc !== blMaster.numeroDoc
      ) {
    ...
    if (validacion.blNoExiste == false) {
      setMensajeAlerta(
        "El numero de BL ingresado ya existe en la base de datos."
      );
      setTipoAlerta("warning");
      setDispatchAlerta(true);
      setTimeout(() => {
    ...
    for (let i = 0; i < blEditable.furgones.length; i++) {
      // Si el BL ingresado tiene espacios
      if (
        blEditable.furgones[i].numeroDoc.includes(" ") ||
        blEditable.furgones[i].numeroDoc.includes("\n")
      ) {
    ...
    const hasTransitoMaritimo = blEditable.furgones.some(
      (furgon) => furgon.status == 1
    );
    if (hasTransitoMaritimo) {
      const todosFurgonesTransitoMaritimo = blEditable.furgones.every(
        (furgon) => furgon.status == 1
      );
    ...
    if (
      ...
      const ordenesUsadas = [
        ...new Set(
          blEditable.furgones.flatMap((furgon) =>
            furgon.materiales.map((item) => item.ordenCompra)
          )
        ),
      ];
      ...
      if (blLlegoAlPais == 0) {
        const { llegadaAlmacen, llegadaDptoImport, llegadaSap } =
          FuncionUpWayDate(annio, mesSinRebajar, dia, 2);
      ...
      // *****Si el BL ya llego al pais, entonces vas a calcular las fechas del ciclo de vida de cada furgon de manera individual, siguiendo el ciclo de vida pero desde el punto colocado por el usuario,(solo hacia adelante, no hacia atras) es decir si el usuario coloca que el furgon esta en almacen, Caeloss colocara en la fecha en puerto '',
        ...
        const furgonesFechaUpDated = blEditable.furgones.map((furgon) => ({
          ...furgon,
        }));
        ...
        for (let i = 0; i < nuevoBLEditable.furgones.length; i++) {
          ...
          // Fecha SAP
            ...
            const { llegadaSap } = FuncionUpWayDate(
              fechaActual.getFullYear(),
              fechaActual.getMonth() + 1, //se le suma uno porque FuncionUpWayDate, espera el mes como si fuera un strin donde febrero es el mes 2, y ella luego lo ajusta
              fechaActual.getDate(),
              4
      ...
      <CajaEncabezado>
        <CajaDetalles>
          <CajitaDetalle>
            ...
            {isEditando == false ? (
              <DetalleTexto title={blMaster.numeroDoc}>
                {blMaster.numeroDoc}
              </DetalleTexto>
            ) : (
              <InputEditable
          ...
          <CajitaDetalle>
            ...
            {isEditando == false ? (
              <DetalleTexto title={blMaster.proveedor}>
                {blMaster.proveedor}
              </DetalleTexto>
            ) : (
              <InputEditable
          ...
          <CajitaDetalle>
            ...
            {isEditando == false ? (
              <DetalleTexto title={blMaster.naviera}>
                {blMaster.naviera}
              </DetalleTexto>
            ) : (
              <InputEditable
          ...
          <CajitaDetalle>
            ...
            {isEditando == false ? (
              <DetalleTexto>
                {blMaster.llegadaAlPais && blMaster.llegadaAlPais.slice(0, 10)}
              </DetalleTexto>
            ) : (
              <InputEditable
                type="date"
            ...
            <Tabla>
              <thead>
                <Filas className="cabeza">
                  ...
                  <CeldaHead className="noFurgon">
                    Contenedor{" "}
                    {`${isEditando || blMaster.estadoDoc == 2 ? "" : "*"}`}
                  </CeldaHead>
                ...
                {isEditando == false
                  ...
                  : blEditable.furgones?.map((furgon, index) => {
                      ...
                      if (blEditable.furgones[index].llegadaSap) {
                        // eslint-disable-next-line no-unused-vars
                        llegadaSap = {
                          annio: blEditable.furgones[index].llegadaSap.slice(
                            6,
                            10
                          ),
                          mes: blEditable.furgones[index].llegadaSap.slice(
                            3,
                            5
                          ),
                          dia: blEditable.furgones[index].llegadaSap.slice(
                            0,
                            2
                          ),
                        };
                      ...
                      return (
                        <Filas key={index} className={"body " + nClases[index]}>
                          ...
                          <CeldasBody>
                            ...
                            >
                              <Opciones disabled value="99999">
                                Seleccione
                              </Opciones>
                              <Opciones disabled value="0">
                                Proveedor
                              </Opciones>
                          ...
                          <CeldasBody>
                            {furgon.llegadaSap &&
                              blEditable.furgones[index].llegadaSap.slice(
                                0,
                                10
                              )}
                          </CeldasBody>
