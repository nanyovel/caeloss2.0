...
export const DetalleUsuarios = ({ useDocByCondition }) => {
  ...
  useEffect(() => {
    if (dbUsuario) {
      ...
      const hasFlete = arrayPrivigelios.find(
        (priv) => priv.code === "fullAccessFletes"
      );
      ...
      if (hasFlete) {
        ...
        arrayPrivParsed = [
          ...arrayPrivigelios,
          {
            ...
            descripcion:
              "Acceso total a la calculadora de fletes, esto incluye poder de escritura sobre los valores de las unidades vehicular.",
            valor: false,
          },
        ];
      }
  ...
  const buscarDoc = (e) => {
    ...
    if (validacion.docExiste == true && validacion.hasNumero == true) {
      navigate("/dashboard/usuarios/" + valorMin);
      setBuscarDocInput("");
    }
  };
  ...
  const funcionAdvert = () => {
    ...
    setMensajeAdvertencia(
      `¿Seguro que desea eliminar el usuario ${userBuscado.userName}?`
    );
    ...
    <Contenedor>
      <SeccionEncabezado>
        <CajaDetalles>
          ...
          <CajaNombreCompleto>
            <TituloNombre>
              {userBuscado ? userBuscado.nombre : "-"}
            </TituloNombre>
          </CajaNombreCompleto>
        </CajaDetalles>
        <CajaDetalles>
          <CajitaDetalle>
            ...
            <DetalleTexto>
              {userBuscado ? userBuscado.userName : ""}
            </DetalleTexto>
          </CajitaDetalle>
          <CajitaDetalle>
            <TituloDetalle>Correo</TituloDetalle>
            <DetalleTexto>{userBuscado ? userBuscado.correo : ""}</DetalleTexto>
          </CajitaDetalle>
          <CajitaDetalle>
          ...
          <CajitaDetalle>
            ...
            <DetalleTexto title={userBuscado?.posicion}>
              {modoEditar ? (
                <InputEditable
                ...
                <CeldasBody className="checkBox">
                  {modoEditar ? (
                    <InputEditable
                      className="checkBox"
                      type="checkbox"
                      checked={userEditable?.privilegios[index]?.valor}
