...
export const Fletes = ({ dbUsuario, userMaster }) => {
  ...
  const handleSelectMun = (e) => {
    ...
    if (value == "Miches") {
      setMensajeAlerta(
        "El destino se considera una ruta peligrosa, se debe tomar la precaución adecuada, y enviar un chofer experimentado."
      );
      setTipoAlerta("error");
  ...
  useEffect(() => {
    ...
    if (puntoSelect.nombrePronvica == municiSelect.provinciaPadre) {
      validacion.origenIgualDestino = setMensajeAlerta(
        "Provincia destino igual a provincia origen, seleccione un radio o indique distancia en kilómetros"
      );
    ...
    if (
      ...
      newResultadoGlobal = {
        ...newResultadoGlobal,
        costo: costo.toLocaleString("es-DO", {
          style: "currency",
          currency: "DOP",
        }),
        precio: Math.round(costo / 0.75).toLocaleString("es-DO", {
          style: "currency",
          currency: "DOP",
        }),
        distancia: distancia,
    ...
    opcionesVehiculos,
    opcionesModalidad,
    opcionesPuntaPartida,
    provinciasParsed,
    distanciaManual,
    valoresMaster,
    datosFijos.totalAnillos,
    datosFijos.horgura,
  ]);
    ...
    <Container>
      ...
      <SeccionEntradaDatos className="oficial">
        {opcionesModalidad[0].select ? (
            ...
            <CajaEntrada>
              ...
              <DataList id="municipios">
                {provinciasParsed
                  ...
                  .map((mun, index) => {
                    return (
                      <Opcion value={mun.label} key={index}>
                        {mun.provinciaPadre}
                      </Opcion>
                    );
                  })}
          ...
          <CajaEntrada className="cajaDistancia">
            <TituloSimple>Ingrese distancia en Km:</TituloSimple>
            <InputSencillo
              placeholder="0"
      ...
      <SeccionEntradaDatos className="seccionCamiones">
        ...
        <CajaCamiones>
          {opcionesVehiculos?.map((vehiculo, index) => {
            return (
              <Card
                className={vehiculo.select == true ? "selected" : ""}
                data-id={index}
                key={index}
                onClick={(e) => {
                  handleVehiculo(e);
      ...
      <SeccionSalidaDatos>
        <CajaInput>
          ...
          <ParrafoSalida>
            {resultadoGlobal.costo == "-" ? "-" : resultadoGlobal.costo}
          </ParrafoSalida>
        </CajaInput>
        ...
        <CajaInput>
          ...
          <ParrafoSalida>
            {resultadoGlobal.precio == "-" ? "-" : resultadoGlobal.precio}
          </ParrafoSalida>
        </CajaInput>
        ...
        <CajaInput>
          ...
          <ParrafoSalida>
            {resultadoGlobal.destino == "-" ? "-" : resultadoGlobal.destino}
          </ParrafoSalida>
        </CajaInput>
        <CajaInput>
          <TextoInput>Provincia</TextoInput>
          <ParrafoSalida>
            {resultadoGlobal.provincia == "-" ? "-" : resultadoGlobal.provincia}
          </ParrafoSalida>
        </CajaInput>
      </SeccionSalidaDatos>

      <SeccionMapa className={rutaPeligrosa ? "peligro" : ""}>
        {rutaPeligrosa ? <TextoPeligro>DESTINO PELIGROSO</TextoPeligro> : ""}
        <div>
          <MapaGoogle
            src={resultadoGlobal.mapa ? resultadoGlobal.mapa : mapaDefault}
          ></MapaGoogle>
        </div>
      </SeccionMapa>
      <Alerta
      ...
      >
        <CajaAviso>
          <TextoAviso>
            Miches se considera una ruta peligrosa, por lo cual es necesario que
            la ruta a utilizar sea a través de Punta Cana, a continuación
          ...
          <ListaDesordenada>
            ...
            <ElementosLista>
              Indicarle al chofer que realice un chequeo de su unidad vehicular;
              frenos, neumáticos, luces y demás.{" "}
            </ElementosLista>
          </ListaDesordenada>
        </CajaAviso>
      </AvisoModal>
